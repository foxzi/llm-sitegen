...
### Важно: заголовки требуют явного цвета

Фреймворки (Bulma, Bootstrap) используют серые цвета для заголовков по умолчанию. На цветных фонах это создаёт плохой контраст.

**Всегда задавай цвет заголовков явно:**

```css
/* Заголовки на светлых поверхностях */
.title { color: var(--on-light) !important; }
.subtitle { color: var(--on-light-muted) !important; }

/* Заголовки на primary поверхностях (CTA) */
.hero.is-link .title { color: var(--on-primary) !important; }
```
...
- [ ] Все заголовки имеют явный цвет (не дефолтный серый)
